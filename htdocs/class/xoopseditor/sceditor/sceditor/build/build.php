...
class SCEditor
{
    ...
    protected function get_commands($file)
    {
        ...
        foreach ($lines as $line) {
            if (false !== ($pos = stripos($line, self::CMD_START))) {
    ...
    protected function get_file($file, $commands)
    {
        ...
        foreach ($lines as $line) {
            ...
            elseif (false !== ($pos = stripos($line, self::CMD_END))) {
                $remove = false;
            } elseif (!$remove) {
                $ret[] = $line;
            }
...
if (isset($_GET['build'])) {
    $editor_cmds           = isset($_GET['ec']) ? array_flip($_GET['ec']) : [];
    $editor_bbcodess       = isset($_GET['eb']) ? array_flip($_GET['eb']) : [];
