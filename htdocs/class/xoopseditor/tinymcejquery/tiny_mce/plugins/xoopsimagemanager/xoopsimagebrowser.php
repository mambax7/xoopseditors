...
if (($isadmin) || ($catreadcount > 0) || ($catwritecount > 0)) {
    ...
    if (!empty($_POST['op']) && 'addfile' == $op) {
        ...
        $uploader = new XoopsMediaUploader(XOOPS_UPLOAD_PATH, ['image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png', 'image/bmp'], $imgcat->getVar('imgcat_maxsize'), $imgcat->getVar('imgcat_maxwidth'), $imgcat->getVar('imgcat_maxheight'));
    ...
    // Add new cat�gorie - start
    if (!empty($_POST['op']) && 'addcat' == $op) {
        if (!$GLOBALS['xoopsSecurity']->check()) {
            redirect_header($current_file . '?target=' . $target, 3, implode('<br />', $GLOBALS['xoopsSecurity']->getErrors()));
        }
        $imgcat_handler =& xoops_gethandler('imagecategory');
    ...
    // Add new cat�gorie - end

    // Update categorie - start
    if (!empty($_POST['op']) && 'updatecat' == $op) {
    ...
    if (!empty($catreadlist)) {
        ...
        for ($i = 0; $i < $catcount; $i++) {
            ...
            if (in_array($imagecategories[$i]->getVar('imgcat_id'), array_keys($catreadlist))) {
                ...
                if ($isadmin && 'C' == $imagecategories[$i]->getVar('imgcat_type')) {
...
if ('listimg' == $op) {
    ...
    foreach (array_keys($images) as $i) {
        ...
        echo '<img id="imageid' . $images[$i]->getVar('image_id') . '" src="' . $image_src . '" alt="' . $images[$i]->getVar('image_nicename', 'E') . '" title="' . $images[$i]->getVar('image_nicename', 'E') . '" onclick="XoopsimagebrowserDialog.insertAndClose(\'imageid' . $images[$i]->getVar(
                'image_id'
            ) . '\');return false;"/>';
        echo '<br />';
        ...
        if ($catwritelist) {
            ...
            echo '<td class="even">';
            echo '<input type="checkbox" name="image_display[]" value="1"' . ((1 == $images[$i]->getVar('image_display')) ? ' checked="checked"' : '') . ' />';
            echo '</td>';
            echo '</tr>';
        }
        echo '</table>';
