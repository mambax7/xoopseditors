...
if ('image' == $insertMode) {
    if (false === $IMConfig['images_enable_styling']) {
        ...
        $num_rows        += 2;
    } elseif (false === $IMConfig['use_color_pickers']) {
        $hidden_fields[] = 'f_backgroundColor';
        $hidden_fields[] = 'f_borderColor';
        $num_rows        += 2;
    }

    if (false === $IMConfig['images_enable_align']) {
...
if ('link' == $insertMode) {
    if (false === $IMConfig['link_enable_target']) {
...
<head>
    <title>Insert <?php echo('image' == $insertMode ? 'Image' : 'File Link') ?></title>
    ...
    <script type="text/javascript">
        ...
        var manager_show_target = <?php echo('link' == $insertMode && $IMConfig['link_enable_target'] ? 'true' : 'false') ?>;
        ...
        $iframe_url = str_replace(['backend.php', 'manager.php'], '', $_SERVER['PHP_SELF']) . $IMConfig['backend_url'];
        ?>
...
<body class="dialog">
<div class="title">Insert <?php echo('image' == $insertMode ? 'Image' : 'File Link') ?></div>
<form action="<?php print htmlspecialchars($IMConfig['backend_url']); ?>" id="uploadForm" method="post" enctype="multipart/form-data">
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td nowrap="nowrap" style="padding:10px;">
                    ...
                    <?php if (false == $IMConfig['safe_mode'] && $IMConfig['allow_new_dir']) { ?>
                    ...
                    <select name="viewtype" id="viewtype" onchange="updateView()">
                        <option value="thumbview" <?php if ('thumbview' == $IMConfig['view_type']) {
                            echo 'selected="selected"';
                        } ?> >Thumbnail View
                        </option>
                        <option value="listview" <?php if ('listview' == $IMConfig['view_type']) {
                            echo 'selected="selected"';
                        } ?> >List View
                        </option>
                    </select>
    ...
    <div id="controls">
        <table class="inputTable">
            <tr>
                <td style="text-align: right;" nowrap="nowrap"><label for="f_url"><?php if ('image' == $insertMode) {
                            echo 'File Name';
                        } else {
                            echo 'URL';
                        } ?></label></td>
                <td colspan="5"><input type="text" id="<?php if ('image' == $insertMode) {
                        echo 'f_url';
                    } else {
                        echo 'f_href';
                    } ?>" class="largelWidth" value=""/></td>
                <td rowspan="<?php echo $num_rows ?>" colspan="2" style="vertical-align: top; text-align: center;"><?php if ('image' == $insertMode) { ?>
                    ...
                    <?php } elseif ('link' == $insertMode && false !== $IMConfig['link_enable_target']) { ?><label for="f_align" id="f_target_label">Target Window</label>
            ...
            <tr>
                ...
                <?php } elseif ($IMConfig['allow_upload']) { ?>
                    <td style="text-align: right;"><label for="upload">Upload</label></td>
                    <td colspan="5">
                        <table cellpadding="0" cellspacing="0" border="0">
                            <tr>
                                <td><input type="hidden" name="MAX_FILE_SIZE" value="<?php echo $max = (('image' == $insertMode ? $IMConfig['max_filesize_kb_image'] : $IMConfig['max_filesize_kb_link']) * 1024); ?>"/>
            ...
            <tr>
                <td><?php if (!empty($hidden_fields)) {
                        foreach ($hidden_fields as $hf) {
                            echo "<input type=\"hidden\" id=\"{$hf}\" name=\"{$hf}\" value=\"\" />";
                        }
                    } ?></td>
                <td colspan="5"><span id="diskmesg"></span></td>
            </tr>
            <tr>
                <td style="text-align: right;"><?php if ('image' == $insertMode) { ?> <label for="f_width">Width</label><?php } ?></td>

                <td><?php if ('image' == $insertMode) { ?> <input type="text" id="f_width" class="smallWidth" value="" onchange="javascript:checkConstrains('width');"/><?php } else {
                        echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                    } ?></td>

                <td rowspan="2"><?php if ('image' == $insertMode) { ?><img src="<?php print $IMConfig['base_url']; ?>img/locked.gif" id="imgLock" width="25" height="32" alt="Constrained Proportions" />
                ...
                <td style="text-align: right;"><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><label for="f_margin">Margin</label><?php } ?></td>

                <td><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><input type="text" id="f_margin" class="smallWidth" value=""/><?php } ?></td>
            </tr>
            <tr>
                <td style="text-align: right;"><?php if ('image' == $insertMode) { ?><label for="f_height">Height</label><?php } ?></td>

                <td class="smallWidth"><?php if ('image' == $insertMode) { ?><input type="text" id="f_height" class="smallWidth" value="" onchange="javascript:checkConstrains('height');"/><?php } ?></td>

                <td style="text-align: right;"><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><label for="f_padding">Padding</label><?php } ?></td>

                <td><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><input type="text" id="f_padding" class="smallWidth" value=""/>
                    <?php } ?></td>

                <?php if ('image' == $insertMode && !empty($IMConfig['use_color_pickers']) && false !== $IMConfig['images_enable_styling']) { ?>
            ...
            <tr>
                <td style="text-align: right;"><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_align']) { ?><label for="f_align">Align</label><?php } ?></td>

                <td colspan="2"><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_align']) { ?>
                ...
                <td style="text-align: right;"><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><label for="f_border">Border</label><?php } ?></td>
                <td><?php if ('image' == $insertMode && false !== $IMConfig['images_enable_styling']) { ?><input type="text" id="f_border" class="smallWidth" value=""/><?php } ?></td>
                <?php if ('image' == $insertMode && !empty($IMConfig['use_color_pickers']) && false !== $IMConfig['images_enable_styling']) { ?>
