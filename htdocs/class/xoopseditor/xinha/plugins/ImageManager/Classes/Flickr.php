...
function flickr_request($method, $params = [])
{
    global $IMConfig;
    ...
    if ($feed) {
        ...
        if (!$feed || !isset($feed['stat']) || ('ok' != $feed['stat'])) {
            print_r($params);
...
function flickr_get_default_usage_id()
{
    ...
    foreach ($usages as $usage => $id) {
        if (flickr_is_default_license($id)) {
            return $id;
        }
    }
...
function flickr_get_user_id($NameOrEmail)
{
    if (preg_match('/@/', $NameOrEmail)) {
        $d = flickr_request('flickr.people.findByEmail', ['find_email' => $NameOrEmail]);
        if ($d) {
            return $d['user']['id'];
        }
    } else {
        $d = flickr_request('flickr.people.findByUsername', ['username' => $NameOrEmail]);
        if ($d) {
            return $d['user']['id'];
        }
    }
