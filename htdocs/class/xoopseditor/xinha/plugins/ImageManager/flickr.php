...
function rip_flickr_data($record)
{
    ...
    $description = '';
    $sizes       = flickr_request('flickr.photos.getSizes', ['photo_id' => $record['id']]);
    $sizes       = $sizes['sizes'];
    $thumbs      = []; // w:h => array( file, file, file)
    $largest     = [0, '', 0, 0];
    $smallest    = [3000 * 3000, '', 3000, 3000]; // Ok, hacky, sosumi

    foreach ($sizes['size'] as $thumb) {
        if ('Square' == $thumb['label']) {
            continue;
        ...
        if (!isset($thumbs["{$thumb['width']}x{$thumb['height']}"])) {
            ...
            if (($thumb['width'] * $thumb['height']) > $largest[0]) {
                $largest = [$thumb['width'] * $thumb['height'], $thumb['width'] . 'x' . $thumb['height'], $thumb['width'], $thumb['height']];
            }

            if (($thumb['width'] * $thumb['height']) < $smallest[0]) {
                $smallest = [$thumb['width'] * $thumb['height'], $thumb['width'] . 'x' . $thumb['height'], $thumb['width'], $thumb['height']];
            }
        }
        ...
        case 'details':
            {
                ...
                <table class="listview">
                    ...
                    foreach ($list as $record) {
                        ...
                        ?>
                        <tr>
                            <th><a href="#" class="thumb" style="cursor: pointer;" onclick="selectImage('<?php echo xinha_js_encode($combinedImage) ?>', '<?php echo xinha_js_encode($title); ?>', <?php echo $largest[2]; ?>, <?php echo $largest[3]; ?>);return false;"
                                   title="<?php echo htmlspecialchars($title); ?> - <?php echo htmlspecialchars($dimensions); ?>" onmouseover="showPreview('<?php echo xinha_js_encode($combinedImage); ?>')"
                                   onmouseout="showPreview(window.parent.document.getElementById('f_url').value)"><?php echo htmlspecialchars($title) ?></a></th>
        ...
        default      :
        {
            foreach ($list as $record) {
                ...
                <div class="thumb_holder" id="holder_<?php echo asc2hex($combinedImage) ?>">
                    <a href="#" class="thumb" style="cursor: pointer;" onclick="selectImage('<?php echo xinha_js_encode($combinedImage) ?>', '<?php echo xinha_js_encode($title); ?>', <?php echo $largest[2]; ?>, <?php echo $largest[3]; ?>);return false;"
                       title="<?php echo htmlspecialchars($title); ?> - <?php echo htmlspecialchars($dimensions); ?>">
