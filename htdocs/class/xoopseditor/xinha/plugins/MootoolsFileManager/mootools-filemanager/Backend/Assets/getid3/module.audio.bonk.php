...
class getid3_bonk extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        while (getid3_bonk::BonkIsValidTagName(substr($possible_bonk_tag, 4, 4), true)) {
            ...
            if (("\x00" != $tag_header_test{0}) || (substr($possible_bonk_tag, 4, 4) != strtolower(substr($possible_bonk_tag, 4, 4)))) {
                throw new getid3_exception('Expecting "�' . strtoupper(substr($possible_bonk_tag, 4, 4)) . '" at offset ' . $bonk_tag_offset . ', found "' . $tag_header_test . '"');
            }
    ...
    private function HandleBonkTags(&$bonk_tag_name)
    {
        ...
        switch ($bonk_tag_name) {
            ...
            case 'META':
                ...
                $meta_tag_entries = floor(((strlen($bonk_data) - 8) - 6) / 8); // BonkData - xxxxmeta - �META
                $offset           = 6;
