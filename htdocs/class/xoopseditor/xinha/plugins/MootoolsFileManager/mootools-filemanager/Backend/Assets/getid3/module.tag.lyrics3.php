...
class getid3_lyrics3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if ('LYRICSEND' == $lyrics3_end) {
            $lyrics3_size    = 5100;
            $lyrics3_offset  = filesize($getid3->filename) - 128 - $lyrics3_size;
            $lyrics3_version = 1;
        } // Lyrics3v2, ID3v1, no APE
        elseif ('LYRICS200' == $lyrics3_end) {
            // LSZ = lyrics + 'LYRICSBEGIN'; add 6-byte size field; add 'LYRICS200'
            $lyrics3_size    = $lyrics3_lsz + 6 + strlen('LYRICS200');
            $lyrics3_offset  = filesize($getid3->filename) - 128 - $lyrics3_size;
            $lyrics3_version = 2;
        } // Lyrics3v1, no ID3v1, no APE
        elseif ('DNESCIRYL' == substr(strrev($lyrics3_id3v1), 0, 9)) {            // strrev('LYRICSEND') = 'DNESCIRYL'
        ...
        elseif ('002SCIRYL' == substr(strrev($lyrics3_id3v1), 0, 9)) {             // strrev('LYRICS200') = '002SCIRYL'
        ...
        if (isset($lyrics3_offset)) {
            ...
            if ('LYRICSBEGIN' != substr($raw_data, 0, 11)) {
                if (false !== strpos($raw_data, 'LYRICSBEGIN')) {
            ...
            switch ($lyrics3_version) {
                case 1:
                    if ('LYRICSEND' == substr($raw_data, strlen($raw_data) - 9, 9)) {
                ...
                case 2:
                    if ('LYRICS200' == substr($raw_data, strlen($raw_data) - 9, 9)) {
                        ...
                        if (isset($parsed_lyrics3['raw']['IND'])) {
                            $i = 0;
                            foreach (['lyrics', 'timestamps', 'inhibitrandom'] as $flagname) {
                        ...
                        foreach (['ETT' => 'title', 'EAR' => 'artist', 'EAL' => 'album', 'INF' => 'comment', 'AUT' => 'author'] as $key => $value) {
    ...
    {
        return '1' == $char ? true : ('0' == $char ? false : null);
    }
}
