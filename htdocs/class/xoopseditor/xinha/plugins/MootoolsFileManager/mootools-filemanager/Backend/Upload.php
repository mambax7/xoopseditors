...
class Upload
{
    ...
    public static function move($file, $to, $options = null)
    {
        ...
        if (is_dir($real)) {
            $to = $real . '/' . ($options['name'] ? $options['name'] : $file['base']) . '.' . $file['ext'];
        }
        ...
        if (!move_uploaded_file($file['tmp_name'], $to)) {
            throw new UploadException(strtolower($_FILES[$file]['error'] <= 2 ? 'size' : (3 == $_FILES[$file]['error'] ? 'partial' : 'nofile')));
        }
     ...
     * @param array  $options
     */
    public function mime($file, $options = [])
    {
        $file    = realpath($file);
        $options = array_merge(
            [
        ...
        if (!$mime && in_array($options['extension'], ['gif', 'jpg', 'jpeg', 'png'])) {
        ...
        if ((!$mime || 'application/octet-stream' == $mime) && $options['extension']) {
            static $mimes;
            if (!$mimes) {
