...
class getid3_zip extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        while ($eocd_search_counter++ < 512) {
            ...
            if (strstr($eocd_search_data, 'PK' . "\x05\x06")) {
                ...
                if (isset($info_zip['compression_method']) && ('store' == $info_zip['compression_method']) && !isset($info_zip['compression_speed'])) {
    ...
    private function ZIPparseLocalFileHeader()
    {
        ...
        if (0x04034B50 != $local_file_header['raw']['signature']) {
        ...
        if ($local_file_header['flags']['data_descriptor_used']) {
            ...
            getid3_lib::ReadSequence(
                'LittleEndian2Int',
                $local_file_header['data_descriptor'],
                $data_descriptor,
                0,
                [
                    'crc_32'            => 4,
    ...
    private function ZIPparseCentralDirectory()
    {
        ...
        if (0x02014B50 != $central_directory['raw']['signature']) {
    ...
    private function ZIPparseEndOfCentralDirectory()
    {
        ...
        if (0x06054B50 != $end_of_central_directory['signature']) {
    ...
    public static function CreateDeepArray($array_path, $separator, $value)
    {
        ...
        if (false !== ($pos = strpos($array_path, $separator))) {
            return [substr($array_path, 0, $pos) => getid3_zip::CreateDeepArray(substr($array_path, $pos + 1), $separator, $value)];
        }
