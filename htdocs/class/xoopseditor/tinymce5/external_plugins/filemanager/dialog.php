...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_SESSION['subfolder']) && false === mb_strpos($_SESSION['subfolder'], '../')
        ...
        && 0 !== mb_strpos($_SESSION['subfolder'], '/')
        && false === mb_strpos($_SESSION['subfolder'], '.')) {
    ...
    $files = array_merge([$prev_folder], [$current_folder], $sorted); ?>
        ...
        echo ' btn-inverse';
    } ?>" id="view0" data-value="0" title="<?php echo lang_View_boxes; ?>"><i class="icon-th <?php if (0 == $view) {
        echo 'icon-white';
    } ?>"></i></button>
                                    <button class="btn tip<?php if (1 == $view) {
        echo ' btn-inverse';
    } ?>" id="view1" data-value="1" title="<?php echo lang_View_list; ?>"><i class="icon-align-justify <?php if (1 == $view) {
        echo 'icon-white';
    } ?>"></i></button>
                                    <button class="btn tip<?php if (2 == $view) {
        echo ' btn-inverse';
    } ?>" id="view2" data-value="2" title="<?php echo lang_View_columns_list; ?>"><i class="icon-fire <?php if (2 == $view) {
        echo 'icon-white';
    } ?>"></i></button>
                                </div>
        ...
        ?><label id="filter" class="btn"><i
                                                class="icon-play"></i></label><?php
    } ?>

                                    <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"/>
                                    <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php if (1 == $_GET['type'] || 3 == $_GET['type']) {
        ?>style="visibility: hidden;" <?php
    } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;"
                                           class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
    ...
    if (!empty($bc)) {
        foreach ($bc as $k => $b) {
            ...
            if ($k == count($bc) - 2) {
                ...
                <li class="pull-right"><a id="refresh" class="btn-small" href="dialog.php?<?php echo $get_params . $subdir . '&' . uniqid('', true) ?>"><i class="icon-refresh"></i></a></li>
        ...
        echo ($descending) ? 'descending' : 'ascending';
                ...
                <ul class="grid cs-style-2 <?php echo 'list-view' . $view; ?>">
                    ...
                    foreach ($files as $file_array) {
                        ...
                        if ('.' === $file || (isset($file_array['extension']) && lang_Type_dir != $file_array['extension']) || ('..' === $file && '' == $subdir) || in_array($file, $hidden_folders) || ('' != $filter && '..' !== $file && false === mb_strpos($file, $filter))) {
                            continue;
                        }
                        ...
                        <li data-name="<?php echo $file ?>" <?php if ('..' === $file) {
                            ...
                            echo 'dir';
                                ...
                                <a class="folder-link" href="dialog.php?<?php echo $get_params . urlencode($src) . '&' . uniqid('', true) ?>">
                            ...
                            echo 'ellipsis';
                        } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $get_params . urlencode($src) . '&' . uniqid('', true) ?>"><?php echo $file; ?></a></h4>
                                    </div>
 ...
 as $nu => $file_array) {
                        ...
                        if ('.' === $file || '..' === $file || is_dir($current_path . $subfolder . $subdir . $file) || in_array($file, $hidden_files) || !in_array(fix_strtolower($file_array['extension']), $ext) || ('' != $filter && false === mb_strpos($file, $filter))) {
                            continue;
                        }
                    ...
                    <li class="ff-item-type-<?php echo $class_ext; ?> file" data-name="<?php echo $file; ?>">
                        <figure data-name="<?php echo $file ?>" data-type="<?php if ($is_img) {
                            ...
                            <figcaption>
                                ...
                                echo 'video';
                            } ?>" title="<?php echo lang_Preview ?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $current_path . $subfolder . $subdir . $file; ?>" href="javascript:void('');"><i class=" icon-eye-open"></i></a>
                                    <?php } else { ?>
